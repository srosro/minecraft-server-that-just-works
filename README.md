...
> **`server/server.properties` can pick up generated credentials.** The server
> rewrites that file on every start, and newer versions add secrets to it —
> `management-server-secret` appeared in 26.x. It has to stay tracked, since it's the
> config this repo ships, so `.gitignore` can't protect it the way it protects
> `plugins/floodgate/key.pem`. **Blank any such value before committing**; the server
> regenerates it at runtime, so blanking costs nothing.
>
> A repo-local `pre-commit` hook would be the obvious enforcement, and it was tried and
> removed: git's `core.hooksPath` is all-or-nothing, so setting it per-repo overrides a
> global one and silences whatever it was pointing at. Doing that here disabled this
> repo's per-commit review tooling outright. The durable fix is to stop tracking a
> server-rewritten file at all — ship a template and generate the live copy — which is
> filed as issue #3.
>
> If such a value was already committed **and pushed**, blanking it afterwards does not
> remove it from history: treat it as disclosed and rotate — delete the line, restart
> the server to regenerate, and assume the old value is public.
